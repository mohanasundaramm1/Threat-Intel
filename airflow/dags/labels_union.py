...
# -------- task --------
def build_union(ds: str, ts: str, **context):
    ...
    if dfs:
        ...
        # Write an empty-but-valid dataset (prevents downstream failures).
        union = pd.DataFrame({
    ...
    union = union.sort_values(["domain", "first_seen", "source"]).drop_duplicates(
        subset=["domain", "first_seen", "source"], keep="first"
    )

    # Idempotent write to ds partition
